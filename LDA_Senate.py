...
# read in senators tweets from csv file
senate_csv = pd.read_csv('senate.csv')
...
print(senate_csv['Tweet'].head())


# In[4]:


print(senate_csv['Tweet'][0])
...
senate_csv['Tweet'] = senate_csv['Tweet'].astype('unicode')
...
cleaned_tweets = [remove_stopwords(token) for token in senate_csv['Tweet']]
...
corpus_senate = [dictionary.doc2bow(tweet) for tweet in lemma_tweets]
...
print('Number of tweets: %d' % len(corpus_senate))
...
top_topics = LDA_model.top_topics(corpus = corpus_senate, topn=5)
